...
async def gpt_dialog(update, context):
    my_message = await send_text(update, context, 'ChatGPT думает над вариантом ответа...')
...
async def date_button(update, context):
    ...
    await send_text(update, context, 'Отличный выбор! Пригасите девушку (парня) на свидание за 5 сообщений')
...
async def hello(update, context):
    if dialog.mode == 'gpt':
        ...
        await send_text(update, context, '*Как* _дела_?')
...
async def hello_button(update, context):
    query = update.callback_query.data
    if query == "start":
        await send_text(update, context, 'Process loaded')
    else:
...
app = ApplicationBuilder().token("7218889233:AAG97DqtLSw9X7DzUO8z6BMWvsvgvc19byA").build()
